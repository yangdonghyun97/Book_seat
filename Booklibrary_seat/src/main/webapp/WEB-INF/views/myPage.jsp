...
<body>
	...
	<section>
		...
		<hr>
		휴대전화 : ${vo.student_tel } <input type="button" onclick="getShow3()"
			value="번호 변경"><br>
		<div id="hide3" style="display: none">
			<div id="msg3"></div>
			전화번호 <input type="text" name="tel" onKeyup="inputPhoneNumber(this);"
				maxlength="13" /> <input type="button" value="변경"
				onclick="telChange()"> <input type="button" value="취소"
				onclick="getHide3()">
		</div>
		<hr>
...
<script>
	...
	function changePw() {
		...
		xhttp.open("POST", "/changePw.do?student_id=" + id + "&student_pw="
				+ pw + "&student_cpw=" + cpw, true);
		xhttp.send();
	}
	...
	function telChange() {
		...
		xhttp.open("POST", "/telchange.do?student_tel=" + phone
				+ "&student_id=" + id, true);
		xhttp.send();
	}
